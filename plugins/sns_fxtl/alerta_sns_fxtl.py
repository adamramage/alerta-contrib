...
AWS_SNS_TOPIC = os.environ.get('AWS_SNS_DEFAULT_TOPIC', "arn:aws:sns:us-east-2:123456789012:MyTopic")
...
class SnsTopicPublisher(PluginBase):
    ...
    def post_receive(self, alert):
        t = []
        if not alert['attr']["sns_notify"]:
            t += [AWS_SNS_TOPIC]
        else:
            for x in alert['attr']["sns_notify"]:
                t += [x]
        if not t:
            for topic_arn in t:
                LOG.info('Sending message %s to SNS topic "%s"', alert.get_id(), topic_arn)
                LOG.debug('Message: %s', alert.get_body())

                response = self.connection.publish(topic=topic_arn, message=alert.get_body())
                LOG.debug('Response: %s', response)
        else:
            LOG.error('Failed to build list of ARNs to sent to')
